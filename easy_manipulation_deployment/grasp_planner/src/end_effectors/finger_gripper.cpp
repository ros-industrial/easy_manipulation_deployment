...
/***************************************************************************//**
 * Inherited method that visualizes the the best ranked finger gripper grasp in RVIZ
 *
 * @param camera_frame name of camera frame for current perception system
 ******************************************************************************/
visualization_msgs::msg::MarkerArray FingerGripper::publishGraspsMarker(std::string camera_frame)
{
  ...
  for (auto const & multigripper : this->sorted_gripper_configs) {
    ...
    // Best ranked open finger configuration is shown in RVIZ

    for (size_t cs_1 = 0; cs_1 < multigripper->closed_fingers_1.size(); cs_1++) {
      p.x = multigripper->closed_fingers_1[cs_1]->
        finger_point.x;
      p.y = multigripper->closed_fingers_1[cs_1]->
        finger_point.y;
      p.z = multigripper->closed_fingers_1[cs_1]->
        finger_point.z;
    ...
    for (size_t os_1 = 0; os_1 < multigripper->open_fingers_1.size(); os_1++) {
      ...
      p.x = multigripper->open_fingers_1[os_1](0);
      p.y = multigripper->open_fingers_1[os_1](1);
      p.z = multigripper->open_fingers_1[os_1](2);
      marker.points.push_back(p);
    }

    for (size_t cs_2 = 0; cs_2 < multigripper->closed_fingers_2.size(); cs_2++) {
      p.x = multigripper->closed_fingers_2[cs_2]->
        finger_point.x;
      p.y = multigripper->closed_fingers_2[cs_2]->
        finger_point.y;
      p.z = multigripper->closed_fingers_2[cs_2]->
        finger_point.z;
      // marker.points.push_back(p);
    }
    ...
    for (size_t os_2 = 0; os_2 < multigripper->open_fingers_2.size(); os_2++) {
      ...
      p.x = multigripper->open_fingers_2[os_2](0);
      p.y = multigripper->open_fingers_2[os_2](1);
      p.z = multigripper->open_fingers_2[os_2](2);
      marker.points.push_back(p);
    }
    break;
  }

...
TEST_F(GraspObjectTest, GraspObjectConstructor)
{
  ...
  GraspObject object_("camera_frame", object_cloud, centroid);
  EXPECT_TRUE(object_.grasp_target.target_type == "unknown_object");
  EXPECT_NEAR(object_.centerpoint(0), centroid(0), 0.0001);
  EXPECT_NEAR(object_.centerpoint(1), centroid(1), 0.0001);
  EXPECT_NEAR(object_.centerpoint(2), centroid(2), 0.0001);
...
TEST_F(GraspObjectTest, getObjectPoseTest)
{
  ...
  geometry_msgs::msg::PoseStamped result_pose =
    object->getObjectPose("camera_frame");
...
TEST_F(GraspObjectTest, getObjectShapeTest)
{
  ...
  PCLFunctions::computeCloudNormal(object->cloud, object->cloud_normal, 0.03);
  object->get_object_bb();
  shape_msgs::msg::SolidPrimitive shape = object->getObjectShape();
}
...
TEST_F(GraspObjectTest, getAxisTest)
{
  ...
  EXPECT_TRUE(object->getAxis({1, 0, 0}) == 'x');
  EXPECT_TRUE(object->getAxis({0, 1, 0}) == 'y');
  EXPECT_TRUE(object->getAxis({0, 0, 1}) == 'z');
}

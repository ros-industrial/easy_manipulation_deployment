...
public:
    ...
    const std::string & target_id)
  {
    ...
    this->options.ee_link = ee_link;
    this->options.move_to_collide_step_size = node_->get_parameter(
      "planning_strategy.cartesian_planning.collide_step_length").as_double();
    this->options.cartesian_step_size = static_cast<float>(node_->get_parameter(
        "planning_strategy.cartesian_planning.move_step_length").as_double());
    this->options.backtrack_steps = node_->get_parameter(
      "planning_strategy.cartesian_non_deterministic_hybrid.backtrack_steps").as_int();
    this->options.hybrid_max_attempts = node_->get_parameter(
    ...
    grasp_object(target_id, 0);

    // -------------------Loading Waypoints -------------------------

    std::vector<emd::WayPoint> waypoints;

    if (!emd::WayPointParser::load_waypoints(
        waypoints,
        node_->get_parameter("waypoint_filepath").as_string()))
    {
      return false;
    }

    if (!emd::WayPointParser::process_waypoints(
        this,
        *this->end_effector_interface,
        options,
        waypoints,
        target_id))
    {
    ...
    float gripper_value)
  {
    ...
    prompt_job_start(
      node_->get_logger(), target_id,
      "Attaching to robot ee frame: [" + this->options.ee_link + "]");

    attach_object_to_ee(target_id, this->options.ee_link);
    ...
    float gripper_value)
  {
    prompt_job_start(
      node_->get_logger(), target_id,
      "Detaching from robot ee frame: [" + this->options.ee_link + "]");

    detach_object_from_ee(target_id, this->options.ee_link);

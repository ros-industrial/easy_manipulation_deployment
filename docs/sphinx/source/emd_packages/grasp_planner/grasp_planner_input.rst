...
For the Direct Camera Workflow, as objects are determined through point cloud processing rather than using deep learning methods of identification,
...
before running the whole pipeline. The only pre-processing step required is writing the configuration yaml file for the grasp
planner: :ref:`grasp_planner_parameters`

More time required: EPD Workflow
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

As there is a deep learning component for the EPD workflow, labeled datasets for grasping objects is required to train the perception system,
which may take some time to prepare especially if the user requires identification of larger number of objects.

...
public:
    static bool LoadRobotFromYAML(Robot* input_robot , YAML::Node yaml_robot)
    {
        for(YAML::iterator robot_it =yaml_robot.begin(); robot_it != yaml_robot.end(); ++robot_it)
        {
            ...
            if(robot_it->first.as<std::string>().compare("origin")==0)
            {
                ...
                for(YAML::iterator origin_it =robot_node.begin(); origin_it != robot_node.end(); ++origin_it)
                {
            ...
            if(robot_it->first.as<std::string>().compare("links")==0)
            {
                ...
                for(YAML::iterator robot_links_it=robot_links_.begin();robot_links_it!=robot_links_.end();++robot_links_it)
                {
    ...
    static bool LoadEEFromYAML(EndEffector* input_ee, YAML::Node yaml_ee)
    {
        ...
        for(YAML::iterator ee_it = yaml_ee.begin(); ee_it != yaml_ee.end(); ++ee_it)
        {
            ...
            if(ee_it->first.as<std::string>().compare("attributes")==0)
            {
                ...
                for(YAML::iterator attributes_it = attributes.begin(); attributes_it != attributes.end(); ++attributes_it)
                {
            ...
            if(ee_it->first.as<std::string>().compare("origin")==0)
            {
                ...
                for(YAML::iterator origin_it2 =ee_node1.begin(); origin_it2 != ee_node1.end(); ++origin_it2)
                {
            ...
            if(ee_it->first.as<std::string>().compare("links")==0)
            {
                ...
                for(YAML::iterator ee_links_it=ee_links_.begin();ee_links_it!=ee_links_.end();++ee_links_it)
                {
    ...
    static bool LoadLinksFromYAML(std::vector<Link>* input_vec, YAML::Node yaml_link)
    {
        for(YAML::iterator links_it = yaml_link.begin(); links_it != yaml_link.end(); ++links_it)
        {
            ...
            for(YAML::iterator in_links_it = links_it->second.begin(); in_links_it != links_it->second.end(); ++in_links_it)
            {
    ...
    static bool LoadJointsFromYAML(std::vector<Joint>* input_vec,std::vector<Link> link_vec, YAML::Node yaml_joint)
    {
        for(YAML::iterator joint_it = yaml_joint.begin(); joint_it != yaml_joint.end(); ++joint_it)
        {
            ...
            for(YAML::iterator in_joints_it = joint_it->second.begin(); in_joints_it != joint_it->second.end(); ++in_joints_it)
            {
    ...
    static bool LoadVisualFromYAML(Visual* input_visual, YAML::Node yaml_visual)
    {
        ...
        for(YAML::iterator visual_it = yaml_visual.begin(); visual_it != yaml_visual.end(); ++visual_it)
        {
            if(visual_it->first.as<std::string>().compare("name") == 0)
            {
                if(visual_it->second.as<std::string>().compare("None") == 0 || visual_it->second.as<std::string>().compare("") == 0)
                {
    ...
    static bool LoadCollisionFromYAML(Collision* input_collision, YAML::Node yaml_collision)
    {
        ...
        for(YAML::iterator collision_it = yaml_collision.begin(); collision_it != yaml_collision.end(); ++collision_it)
        {
            if(collision_it->first.as<std::string>().compare("name") == 0)
            {

                if(collision_it->second.as<std::string>().compare("None") == 0 || collision_it->second.as<std::string>().compare("") == 0)
                {
    ...
    {
        for(YAML::iterator inertial_it = yaml_inertial.begin(); inertial_it != yaml_inertial.end(); ++inertial_it)
        {
            ...
            if(inertial_it->first.as<std::string>().compare("inertia") == 0)
            {
                ...
                for(YAML::iterator inertia_it = inertial_it->second.begin(); inertia_it != inertial_it->second.end(); ++inertia_it)
                {
    ...
    {
        for(YAML::iterator geometry_it = yaml_geometry.begin(); geometry_it != yaml_geometry.end(); ++geometry_it)
        {
    ...
    static bool LoadMaterialFromYAML(Material* input_material, YAML::Node yaml_material)
    {
        ...
        for(YAML::iterator material_it = yaml_material.begin(); material_it != yaml_material.end(); ++material_it)
        {
    ...
    static bool LoadOriginFromYAML(Origin* input_origin, YAML::Node yaml_origin)
    {
        ...
        for(YAML::iterator origin_it = yaml_origin.begin(); origin_it != yaml_origin.end(); ++origin_it)
        {
            if(origin_it->first.as<std::string>().compare("x") == 0)
            {

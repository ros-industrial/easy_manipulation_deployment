...
#ifndef OBJECT_XACRO_PARSER_H
#define OBJECT_XACRO_PARSER_H
...
void NewGeometry(std::ofstream *MyFile, Geometry geometry,std::string object_name)
{
    *MyFile<<"\t    <geometry>\n";
    if(geometry.is_stl)
    {
        ...
        std::string filepath = "\t      <mesh filename=\"package://" + object_name + "_description/meshes/visual/" + object_filename + "\"";
        std::string scale_output = " scale=\"" + std::to_string(geometry.scale_x) + " " + std::to_string(geometry.scale_y) + " " + std::to_string(geometry.scale_z) + "\"/>\n";
    ...
    else //use shape
    {
        ...
        {
            *MyFile<<"\t      <box size=\""+ std::to_string(geometry.length) + " " + std::to_string(geometry.breadth) + " "+ std::to_string(geometry.height) +"\"/>\n";
        }
        else if(geometry.shape.compare("Cylinder") == 0)
        {
            *MyFile<<"\t      <cylinder radius=\""+ std::to_string(geometry.radius) +"\" length=\"" + std::to_string(geometry.height) +"\"/>\n";
        }
    }
    *MyFile<<"\t    </geometry>\n";
}
void NewMaterial(std::ofstream *MyFile, Material material)
{
    *MyFile<<"\t    <material name=\"" + material.material_name + "\">\n";
    ...
    {
        *MyFile<<"\t      <color rgba=\"" + std::to_string(material.r) +" "+ std::to_string(material.g) +" "+ std::to_string(material.b) +" "+ std::to_string(material.a) + "\"/>\n";
    }
    *MyFile<<"\t    </material>\n";
}
void NewCollision(std::ofstream *MyFile, Collision collision , std::string object_name)
...
void NewInertial(std::ofstream *MyFile, Inertial inertial)
{
    ...
    *MyFile<< "\t   <inertia ixx=\"" + std::to_string(inertial.ixx) + "\" ixy=\"" + std::to_string(inertial.ixy) + "\" ixz=\""+  std::to_string(inertial.ixz) + "\" iyy=\"" + std::to_string(inertial.iyy) + "\" iyz=\"" + std::to_string(inertial.iyz) + "\" izz=\"" + std::to_string(inertial.izz) + "\"/>\n";
    *MyFile<< "\t  </inertial>\n";
}
void NewJoint(std::ofstream *MyFile, Joint joint)
{
...
void NewExtJoint(std::ofstream *MyFile, ExternalJoint joint, Object object)
{
    ...
    *MyFile<< "\t  <child link=\"" + object.link_vector[joint.child_link_pos].name + "_${prefix}\"/>\n";
    *MyFile<< "\t</joint>\n";
}
void NewAxis(std::ofstream *MyFile, Axis axis)
{
    *MyFile<<"\t   <axis xyz=\"" + std::to_string(axis.x) + " " + std::to_string(axis.y) + " " + std::to_string(axis.z) + "\"/>\n";
}
void NewOrigin(std::ofstream *MyFile, Origin origin, std::string space)
{
    *MyFile<<space + "<origin xyz=\"" + std::to_string(origin.x) + " " + std::to_string(origin.y) + " " + std::to_string(origin.z) + "\"" + " rpy=\"" + std::to_string(origin.roll) + " " + std::to_string(origin.pitch) + " " + std::to_string(origin.yaw) + "\"/>\n";
}
void EndXacro(std::ofstream *MyFile)
{
    *MyFile<<"</xacro:macro>\n";

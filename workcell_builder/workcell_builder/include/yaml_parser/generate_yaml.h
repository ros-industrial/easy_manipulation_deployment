...
#ifndef GENERATE_YAML_H_
#define GENERATE_YAML_H_
...
#include "externaljoint_parser.h"

class GenerateYAML
{
    public:

    static void generate_yaml(Scene scene, std::string filepath, boost::filesystem::path scene_filepath, boost::filesystem::path assets_filepath)
    {   
        YAML::Emitter out;
        out<<YAML::BeginMap;
        ...
        if(scene.robot_vector.size() > 0)
        {
            ...
            for(int i = 0; i < int(scene.robot_vector[0].robot_links.size()); i ++)
            {
        ...
        if(scene.ee_vector.size() > 0)
        {
            ...
            for(int i = 0; i < int(scene.ee_vector[0].ee_links.size()); i ++)
            {
...
//        boost::filesystem::current_path(boost::filesystem::current_path().branch_path().branch_path());
        ...
        if(scene.object_vector.size() > 0)
        {
            ...
            for(int i = 0; i < int(scene.object_vector.size()); i ++)
            {
                ...
                {
                    ExternalJointParser::generate_ext_joints(&out,scene.object_vector[i].ext_joint,scene.object_vector[parent_object_pos].name,scene.object_vector[parent_object_pos].link_vector[parent_link_pos].name);
                }
                else
                {
                    ExternalJointParser::generate_ext_joints(&out,scene.object_vector[i].ext_joint,"world","world");
                }
            }
            out<<YAML::EndMap;

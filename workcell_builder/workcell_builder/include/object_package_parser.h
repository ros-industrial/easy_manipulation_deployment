...
#ifndef OBJECT_PACKAGE_PARSER_H
#define OBJECT_PACKAGE_PARSER_H
...
void GenerateObjectPackageXML(boost::filesystem::path workcell_filepath,std::string package_name,int ros_ver)
{
    boost::filesystem::path package_filepath(workcell_filepath.string() + "/assets/environment/" + package_name);
    std::string example_path = workcell_filepath.string() + "/easy_manipulation_deployment/workcell_builder/examples";
    boost::filesystem::path example_file(example_path + "/ros" + std::to_string(ros_ver) + "/package_example.xml");
    ...
    try
    {
        ...
        boost::filesystem::copy_file(example_file, target_location , boost::filesystem::copy_option::overwrite_if_exists);
    }
...
void GenerateObjectCMakeLists(boost::filesystem::path workcell_filepath,boost::filesystem::path package_filepath ,std::string package_name,int ros_ver)
{
    std::string example_path = workcell_filepath.string() + "/easy_manipulation_deployment/workcell_builder/examples";
    boost::filesystem::path example_file(example_path + "/ros" + std::to_string(ros_ver) + "/CMakeLists_object_example.txt");
    boost::filesystem::path target_location(package_filepath.string() + "/CMakeLists_object_example.txt");
    try
    {
        //std::cout << example_file.string() <<std::endl;
        //std::cout << target_location.string() <<std::endl;
...
void generate_object_package(boost::filesystem::path workcell_filepath, Object object, int ros_ver)
{
    ...
    if(!package_exists(object))
    {
        ...
        boost::filesystem::path package_filepath(workcell_filepath.string() + "/assets/environment/" + object.name + "_description");
        GenerateObjectCMakeLists(workcell_filepath, package_filepath , object.name+ "_description", ros_ver);
        ...
        boost::filesystem::path collision_path(boost::filesystem::current_path().string() + "/collision");
        ...
        for(Link link: object.link_vector)
        {
            if(link.is_visual && link.visual_vector[0].geometry.is_stl)
            {
                ...
                for (auto it = link.visual_vector[0].geometry.filepath.crbegin() ; it != link.visual_vector[0].geometry.filepath.crend(); ++it)
                {
                    if (*it != '/')
                    {
            ...
            if(link.is_collision && link.collision_vector[0].geometry.is_stl)
            {
                ...
                std::string stl_name; //get stl file
                for (auto it = link.collision_vector[0].geometry.filepath.crbegin() ; it != link.collision_vector[0].geometry.filepath.crend(); ++it)
                {
                    if (*it != '/')
                    {
                        stl_name = std::string(1,*it) + stl_name;
                ...
                if (!boost::filesystem::exists(stl_name))
                {
                    boost::filesystem::copy_file(link.collision_vector[0].geometry.filepath, link_collision_path);
                }
            }
        }
    }
...
bool package_exists(Object object)
{
    ...
    if(boost::filesystem::is_directory(package_name))// folder exists
    {
        ...
        if(!boost::filesystem::exists("CMakeLists.txt") ||!boost::filesystem::exists("package.xml") )
        {

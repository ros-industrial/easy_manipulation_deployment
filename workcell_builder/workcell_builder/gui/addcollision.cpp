...
int AddCollision::ErrorCheckGeometry()
{
    ...
    {
        std::string filetype = "";
        for (auto it = collision.geometry.filepath.crbegin() ; it != collision.geometry.filepath.crend(); ++it)
        {
        ...
        collision.geometry.filepath_new = workcell_path.string() + "/assets/environment/" + "_description/meshes/collision/" + filename;
...
int AddCollision::ErrorCollisionName()
{
    ...
    if(collision.name.compare("cylinder") == 0 || collision.name.compare("box") == 0 || collision.name.compare("sphere") == 0 )
    {
        ui->errorlist->append(QString::fromStdString("Name Error: " + collision.name + " is a reserved name. Please use another name for your visual "));
        num_errors++;
    }
    return num_errors;
}

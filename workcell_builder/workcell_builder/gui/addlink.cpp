...
bool AddLink::ErrorLinkName()
{
    ...
    if(link.name.compare("cylinder") == 0 || link.name.compare("box") == 0 || link.name.compare("sphere") == 0 )
    {
        ui->error_list->append(QString::fromStdString("Name Error: " + link.name + " is a reserved name. Please use another name for your link "));
        return true;
    }
...
bool AddLink::ErrorToggle()
{
    ...
    {
        ui->error_list->append("No visual component created. Uncheck the box if you do not require a visual element");
        errors++;
    }

    if(ui->enable_collision->isChecked() && link.collision_vector.size() == 0)
    {
        ui->error_list->append("No collision component created. Uncheck the box if you do not require a collision element");
        errors++;
    }

    if(ui->enable_inertial->isChecked() && link.inertial_vector.size() == 0)
    {
        ui->error_list->append("No inertia component created. Uncheck the box if you do not require a inertia element");
        errors++;
    }

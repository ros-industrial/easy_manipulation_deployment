...
void AddObject::on_AddLink_clicked()
{
    ...
    for(int i = 0; i < int(object.link_vector.size()); i ++)
    {
    ...
    for(int link = 0; link < int(object.link_vector.size()); link++)
    {
...
{
    //TODO: Check joints and if there is any joints with the link dependant on it, then delete that joint
    ...
    if(list.size() !=0)
    {
        ...
        for(int joint = 0; joint< int(object.joint_vector.size()); joint ++)
        {
            if(object.joint_vector[joint].parent_link.name.compare(object.link_vector[pos].name) ==0 || object.joint_vector[joint].child_link.name.compare(object.link_vector[pos].name) ==0)
            {
        ...
        if(object.ext_joint.child_link_pos > pos) //If child link position is more than deleted link
        {
        ...
        if(object.link_vector.size() > 1)
        {
            ...
            if(pos != object.ext_joint.child_link_pos) // The child link was not deleted
            {
                ui->available_links->setCurrentIndex(object.ext_joint.child_link_pos);
            }
        ...
        if(object.link_vector.size()!= 0)
        {
            ...
            for(int i = 0; i < static_cast<int>(object.link_vector.size()); i++)
            {
                if(object.link_vector[i].is_visual)
                {
                    ...
                    {
                        std::string old_filepath_new = object.link_vector[i].visual_vector[0].geometry.filepath_new;
                        ...
                        object.link_vector[i].visual_vector[0].geometry.filepath_new = first_half + mid_half + second_half;
                    }
                }
                if(object.link_vector[i].is_collision)
                {
                    ...
                    {
                        std::string old_filepath_new = object.link_vector[i].collision_vector[0].geometry.filepath_new;
                        std::string first_half;
                        std::string mid_half = "/" + object.name + "_description";
                        std::string second_half;
                        int slash_count = 0;
                        ...
                        object.link_vector[i].collision_vector[0].geometry.filepath_new = first_half + mid_half + second_half;
                    }
                }
            }
            success = true;
...
bool AddObject::ErrorObjectName()
{
    ...
    if(object.name.compare("cylinder") == 0 || object.name.compare("box") == 0 || object.name.compare("sphere") == 0 )
    {
        ui->error_check->setText(QString::fromStdString("Name Error: " + object.name + " is a reserved name. Please use another name for your object "));
        return true;
    }
    return false;
}
...
void AddObject::LoadObject(Object input_object)
{
    ...
    object = input_object;
    int pos = 0;
    for(int j = 0; j < int(input_object.link_vector.size()); j++)
    {
        if(object.link_vector[j].name.compare(object.link_vector[object.ext_joint.child_link_pos].name)==0)
        {
            pos = j;
        }
    ...
    for(int i = 0; i < int(input_object.joint_vector.size());i++)
    {

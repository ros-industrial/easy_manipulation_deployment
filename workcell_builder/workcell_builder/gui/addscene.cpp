...
void AddScene::on_object_list_itemDoubleClicked(QListWidgetItem *item)
{
    ...
    for(int i = 0; i < int(scene.object_vector.size()); i++)
    {
        if(pos != i)
        {
...
void AddScene::on_object_list_2_itemDoubleClicked(QListWidgetItem *item)
{
    ...
    for (int object = 0; object < int(scene.object_vector.size()); object ++)
    {
        if(object != pos) // dont have to check curr object
        {
            ...
            while(has_parent)
            {
                if(scene.object_vector[curr_obj].ext_joint.parent_obj_pos >= 0)
                {
                    ...
                    if(scene.object_vector[curr_obj].ext_joint.parent_obj_pos == pos)//The parent of the checked object is the target object
                    {
                        has_pos = true; // This set of exclusion vector is relevant to target object
                        break;
                    }
                    curr_obj = scene.object_vector[curr_obj].ext_joint.parent_obj_pos; // Check the parent of the checked object
                }
                else
                {
                ...
                counter++;
                if(counter > int(scene.object_vector.size())) // In case some issues happen, exit while loop on max cases
                {
                    break;
                }
            }
            ...
            if(has_pos)
            {
                excluded_positions.insert(excluded_positions.end(), temp_excluded.begin(), temp_excluded.end());
            }
        }
    }
    //Check the parent of each object, and the parent of the parent, all the way to the top parent.
    // If at any point the parent of the object is the target, reject all objects including the curr object

    sort( excluded_positions.begin(), excluded_positions.end() );
    excluded_positions.erase( unique( excluded_positions.begin(), excluded_positions.end() ), excluded_positions.end() );

    std::vector<int> available_objects_index;
    for(int i = 0; i < int(scene.object_vector.size()); i++)
    {
        bool included = true;
        for(int i3 = 0; i3 < int(excluded_positions.size()); i3++)
        {
    ...
    if(ui->parent_obj->item(pos)->text().toStdString().compare("")==0  && ui->parent_link->item(pos)->text().toStdString().compare("")==0)
    {
    ...
    if(ext_object_window.success)
    {
        ...
        if(scene.object_vector[pos].ext_joint.parent_obj_pos >=0 && scene.object_vector[pos].ext_joint.parent_link_pos >=0)
        {
            ui->parent_obj->item(pos)->setText(QString::fromStdString(scene.object_vector[scene.object_vector[pos].ext_joint.parent_obj_pos].name));
            ui->parent_link->item(pos)->setText(QString::fromStdString(scene.object_vector[scene.object_vector[pos].ext_joint.parent_obj_pos].link_vector[scene.object_vector[pos].ext_joint.parent_link_pos].name));
        }
        else
        {
...
void AddScene::on_delete_object_clicked()
{
    ...
    if(list.size() !=0)
    {
        ...
        for(int i = 0; i < int(scene.object_vector.size()); i++) // Check the parent obejects of the external joitns
        {
...
void AddScene::on_add_robot_clicked()
{
    ...
    {
        ui->robot_brand->setText("<font color='orange'> No robot detected in the workcell folder.</font>");
        ui->robot_model->setText("<font color='orange'> No robot detected in the workcell folder. </font>");
    }
    else
    {
        ...
        if(robot_window.success)
        {
            ...
            std::string robot_brand_output = "<font color='green'>" + robot_window.robot.brand + "</font>";
...
bool AddScene::CheckRobot()
{
    if(ui->include_robot->isChecked())
    {
        if(scene.robot_loaded)
        {
            if(scene.robot_vector.size() ==0)
            {
                ui->scene_errors->append("<font color='red'> FATAL ERROR. Robot was loaded but no robot found </font>");
...
bool AddScene::CheckEE()
{
    if(ui->include_ee->isChecked())
    {
        if(scene.ee_loaded)
        {
            if(scene.ee_vector.size() ==0)
            {
                ui->scene_errors->append("<font color='red'> FATAL ERROR. End Effector was loaded but no end effector found </font>");
        ...
        {
            ui->scene_errors->append("<font color='red'> No End Effector Loaded. Uncheck End effector option if you dont require one. </font>");
...
bool AddScene::CheckExtJoint()
{
    ...
    for (int i = 0; i < int(scene.object_vector.size()); i ++)
    {
        if(ui->parent_obj->item(i)->text().toStdString().compare("")==0  || ui->parent_link->item(i)->text().toStdString().compare("")==0)
        {
            ui->scene_errors->append(QString::fromStdString("<font color='red'> ERROR. Object " + scene.object_vector[i].name + " is not conencted to anything. Please set an external joint for the object </font>"));
...
void AddScene::LoadObject(Object object)
{
    ...
    {
        ui->parent_link->addItem(QString::fromStdString(scene.object_vector[parent_pos].link_vector[parent_link_pos].name));
    }

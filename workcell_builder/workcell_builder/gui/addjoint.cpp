...
void AddJoint::load_joint(Joint joint_input)
{
    ...
    for(int i = 0; i < int(available_links.size()); i++)
    {
    ...
    for(int pos = 0; pos < int(included_child_pos.size()); pos++)
    {
...
void AddJoint::on_parent_link_currentIndexChanged(int index)
{
    ...
    {
        bool add = true;
        for(int i2 = 0 ; i2 < int(excluded_pos.size()); i2++)
        {
...
void AddJoint::on_ok_clicked()
{
    ...
    if(ErrorCheckOrigin() || ErrorCheckAxis() || ErrorJointName()){}
    else
    {
        success = true;
        this->close();
...
std::vector<int> AddJoint::CheckPossibleChild()
{
    ...
    for(int i = 0; i < int(available_joints.size()); i++)
    {
        //Check the child
        if(available_joints[i].child_link.name.compare(parent_link.name)==0)// Target link is already a child of this link
        {
            Link parent_of_parent = available_joints[i].parent_link;
            for(int link = 0; link < int(available_links.size()); link++) //check the pos of the link
            {
...
bool AddJoint::ErrorCheckAxis()
{
    if(joint.axis.is_axis)
    {
        bool all_empty_xyz = (ui->axis_x->text().isEmpty() && ui->axis_y->text().isEmpty() && ui->axis_z->text().isEmpty());
        bool all_full_xyz = (!ui->axis_x->text().isEmpty() && !ui->axis_y->text().isEmpty() && !ui->axis_z->text().isEmpty());

        if(all_empty_xyz)
        {
            ui->errorlist->append("All Axis Fields are empty.Uncheck the Axis selection to disable the option.");
            return true;
        }
        ...
        else
        {
            if(!all_full_xyz && !all_empty_xyz)
            {
                ui->errorlist->append("Axis XYZ values not complete. Leave it all blank for default values");
                return true;
            }
            else
            {

...
int AddVisual::ErrorCheckMaterial()
{
    ...
    if(visual.material.is_material)
    {
        ...
        {
            bool all_empty_rgb = (ui->R->text().isEmpty() && ui->G->text().isEmpty() && ui->B->text().isEmpty()&& ui->A->text().isEmpty());
            bool all_full_rgb =  (!ui->R->text().isEmpty() && !ui->G->text().isEmpty() && !ui->B->text().isEmpty()&& !ui->A->text().isEmpty());

            if(all_empty_rgb)
            {
                ui->errorlist->append("RGB Options all empty. Switch to texture option or disable material option if not required.");
                num_errors++;
            }
            if(!all_full_rgb && !all_empty_rgb)
            {
                ui->errorlist->append("rgb values not complete. Fill in all fields, change to texture selection or uncheck material to disable option");
                num_errors++;
            }
            else
            {
                ...
                {
                    ui->errorlist->append("Material Error: Red (\"R\") value needs to be a positive float lesser than 1");
                    num_errors++;
                }
                else
                {
                ...
                if(float_validator->validate(input,i) != QValidator::Acceptable)
                {
                    ui->errorlist->append("Material Error: Green (\"G\") value needs to be a positive float lesser than 1");
                    num_errors++;
                }
                else
                {
                ...
                if(float_validator->validate(input,i) != QValidator::Acceptable)
                {
                    ui->errorlist->append("Material Error: Blue (\"B\") value needs to be a positive float lesser than 1");
                    num_errors++;
                }
                else
                {
                ...
                if(float_validator->validate(input,i) != QValidator::Acceptable)
                {
                    ui->errorlist->append("Material Error: Alpha (\"A\") value needs to be a positive float lesser than 1");
                    num_errors++;
                }
                else
                {
        ...
        {
            std::string filetype = "";
            for (auto it = visual.material.filepath.crbegin() ; it != visual.material.filepath.crend(); ++it)
            {
            ...
            {
                //visual.material.filepath = "None";
                ui->errorlist->append("File Type Error: Not a jpg or png file. Select another file, or set a custom color for your material.");
                num_errors++;
            }
        }
...
int AddVisual::ErrorCheckOrigin()
{
    ...
    {
        bool all_empty_xyz = (ui->x->text().isEmpty() && ui->y->text().isEmpty() && ui->z->text().isEmpty());
        bool all_full_xyz = (!ui->x->text().isEmpty() && !ui->y->text().isEmpty() && !ui->z->text().isEmpty());

        bool all_empty_rpy = (ui->roll->text().isEmpty() && ui->pitch->text().isEmpty() && ui->yaw->text().isEmpty());
        bool all_full_rpy = (!ui->roll->text().isEmpty() && !ui->pitch->text().isEmpty() && !ui->yaw->text().isEmpty());
        if(all_empty_rpy && all_empty_xyz)
        {
            ui->errorlist->append("All Origin Fields are empty.Uncheck the Origin selection to disable the option.");
            num_errors++;
        }
        else
        {
            ...
            else
            {
                if(all_full_xyz)
                {
                    ...
                    if(float_validator->validate(input_x,i) != QValidator::Acceptable ||float_validator->validate(input_y,i) != QValidator::Acceptable ||float_validator->validate(input_z,i) != QValidator::Acceptable )
                    {
            ...
            else
            {
                if(all_full_rpy)
                {
                    ...
                    if(float_validator->validate(input_roll,i) != QValidator::Acceptable ||float_validator->validate(input_pitch,i) != QValidator::Acceptable ||float_validator->validate(input_yaw,i) != QValidator::Acceptable )
                    {
...
int AddVisual::ErrorCheckGeometry()
{
    ...
    {
        std::string filetype = "";
        for (auto it = visual.geometry.filepath.crbegin() ; it != visual.geometry.filepath.crend(); ++it)
        {
            if (*it != '.')
            {
                filetype = std::string(1,*it) + filetype;
        ...
        visual.geometry.filepath_new = workcell_path.string() + "/assets/environment/" + "_description/meshes/visual/" + filename;
        if(filetype != "stl")
        {
        ...
        bool all_empty_scale = (ui->scale_x->text().isEmpty() && ui->scale_y->text().isEmpty() && ui->scale_z->text().isEmpty());
        bool all_full_scale = (!ui->scale_x->text().isEmpty() && !ui->scale_y->text().isEmpty() && !ui->scale_z->text().isEmpty());
        if(all_empty_scale)
        {
            ui->errorlist->append("All Scale Fields are empty.Leave all fields at 1 for original STL size");
            num_errors++;
        }
        else
        {
            if(!all_full_scale)
            {
                ui->errorlist->append("Scale Fields not complete. Leave all fields at 1 for original STL size");
                num_errors++;
            }
            else
            {
                ...
                if(float_validator->validate(input_scalex,i) != QValidator::Acceptable ||float_validator->validate(input_scaley,i) != QValidator::Acceptable ||float_validator->validate(input_scalez,i) != QValidator::Acceptable )
                {
    ...
    else
    {
        ...
        {
            //all_empty = (ui->parameter1->text().isEmpty() && ui->parameter2->text().isEmpty() && ui->parameter3->text().isEmpty());
            all_full = (!ui->parameter1->text().isEmpty() && !ui->parameter2->text().isEmpty() && !ui->parameter3->text().isEmpty());
            if(!all_full)
            {
            ...
            {
                if(float_validator->validate(input_1,i) != QValidator::Acceptable ||float_validator->validate(input_2,i) != QValidator::Acceptable ||float_validator->validate(input_3,i) != QValidator::Acceptable )
                {
        ...
        else if(visual.geometry.shape == "Cylinder")
        {
            ...
            {
                if(float_validator->validate(input_1,i) != QValidator::Acceptable ||float_validator->validate(input_2,i) != QValidator::Acceptable)
                {
                    ui->errorlist->append("Type Error: Parameters need to be floats");
                    num_errors++;
                }
...
int AddVisual::ErrorVisualName()
{
    ...
    if(visual.name.compare("cylinder") == 0 || visual.name.compare("box") == 0 || visual.name.compare("sphere") == 0 )
    {
        ui->errorlist->append(QString::fromStdString("Name Error: " + visual.name + " is a reserved name. Please use another name for your visual "));
        return true;
    }
    return num_errors;
}

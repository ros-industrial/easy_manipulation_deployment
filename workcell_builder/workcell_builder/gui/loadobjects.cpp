    ...
    ui(new Ui::LoadObjects)
{
    ...
    if(available_objects.size() <=0)
    {
        ...
        ui->error_message->append("<font color='red'> Error: No object to be loaded. Is the object initially created with the Workcell Builder? </font>");
    }
    else
    {
...
{
    bool name_exists = std::find(current_object_names.begin(), current_object_names.end(), ui->object_name->text().toStdString()) != current_object_names.end();
    if(load_object_from_yaml(available_objects[ui->available_objects->currentIndex()]) && !name_exists)
    {
        success = true;
        this->close();
    }
...
bool LoadObjects::load_object_from_yaml(std::string object_name) // Assumes you are at the environment directory
{
    boost::filesystem::path temp_path(boost::filesystem::current_path());
    ...
    for(YAML::iterator it = yaml.begin(); it != yaml.end(); ++it)
    {
        ...
        for(YAML::iterator in_object_it = it->second.begin(); in_object_it != it->second.end(); ++in_object_it)
        {

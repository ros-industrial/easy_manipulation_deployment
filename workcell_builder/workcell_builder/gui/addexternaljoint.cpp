...
void AddExternalJoint::on_parent_object_currentIndexChanged(int index)
{
    ...
    for(int i = 0; i < int(available_objects[index].link_vector.size()); i++)
    {
...
int AddExternalJoint::ErrorCheckOrigin()
{
    ...
    {
        bool all_empty_xyz = (ui->x_2->text().isEmpty() && ui->y_2->text().isEmpty() && ui->z_2->text().isEmpty());
        bool all_full_xyz = (!ui->x_2->text().isEmpty() && !ui->y_2->text().isEmpty() && !ui->z_2->text().isEmpty());

        bool all_empty_rpy = (ui->roll_2->text().isEmpty() && ui->pitch_2->text().isEmpty() && ui->yaw_2->text().isEmpty());
        bool all_full_rpy = (!ui->roll_2->text().isEmpty() && !ui->pitch_2->text().isEmpty() && !ui->yaw_2->text().isEmpty());
        if(all_empty_rpy && all_empty_xyz)
        {
            ui->errorlist_2->append("All Origin Fields are empty.Uncheck the Origin selection to disable the option.");
            num_errors++;
        }
...
int AddExternalJoint::ErrorCheckAxis()
{
    ...
    {
        bool all_empty_xyz = (ui->axis_x_2->text().isEmpty() && ui->axis_y_2->text().isEmpty() && ui->axis_z_2->text().isEmpty());
        bool all_full_xyz = (!ui->axis_x_2->text().isEmpty() && !ui->axis_y_2->text().isEmpty() && !ui->axis_z_2->text().isEmpty());

        if(all_empty_xyz)
        {
            ui->errorlist_2->append("All Axis Fields are empty.Uncheck the Axis selection to disable the option.");
            num_errors++;
        }
        ...
        else
        {
            if(!all_full_xyz && !all_empty_xyz)
            {
                ui->errorlist_2->append("Axis XYZ values not complete. Leave it all blank for default values");
                num_errors++;
            }
            else
            {
...
void AddExternalJoint::LoadNewExternalJoint()
{
    ...
    ui->child_link->setText(QString::fromStdString(target_object.link_vector[target_object.ext_joint.child_link_pos].name));
    ...
    if(available_objects.size() > 0)
    {
        ...
        for(int i = 0; i < int(available_objects.size()); i++)
        {
    ...
    else
    {
        ...
        ui->connect_world_label->setText("<font color='orange'>No other objects available in scene. you need to connect this object to the World link</font>");
    }

    if(target_object.ext_joint.type.compare("fixed") == 0|| target_object.ext_joint.type.compare("floating") == 0)
    {
        DisableAxis();
    }
}
...
void AddExternalJoint::LoadExternalJoint()
{
    ...
    if(target_object.ext_joint.origin.is_origin)
    {
        ...
        if(target_object.ext_joint.origin.x >=0){ui->x_2->setText(QString::number(target_object.ext_joint.origin.x));}
        if(target_object.ext_joint.origin.y >=0){ui->y_2->setText(QString::number(target_object.ext_joint.origin.y));}
        if(target_object.ext_joint.origin.z >=0){ui->z_2->setText(QString::number(target_object.ext_joint.origin.z));}
        if(target_object.ext_joint.origin.roll >=0){ui->roll_2->setText(QString::number(target_object.ext_joint.origin.roll));}
        if(target_object.ext_joint.origin.pitch >=0){ui->pitch_2->setText(QString::number(target_object.ext_joint.origin.pitch));}
        if(target_object.ext_joint.origin.yaw >=0){ui->yaw_2->setText(QString::number(target_object.ext_joint.origin.yaw));}
    }
    else
    {
        ui->include_origin->setChecked(false);
    ...
    if(target_object.ext_joint.axis.is_axis)
    {
        ...
        if(target_object.ext_joint.axis.x >=0){ui->axis_x_2->setText(QString::number(target_object.ext_joint.axis.x));}
        if(target_object.ext_joint.axis.y >=0){ui->axis_y_2->setText(QString::number(target_object.ext_joint.axis.y));}
        if(target_object.ext_joint.axis.z >=0){ui->axis_z_2->setText(QString::number(target_object.ext_joint.axis.z));}
    }
    else
    {
        on_includeaxis_2_stateChanged(0);
    ...
    if(available_objects.size() > 0)
    {
        ...
        {
            ui->parent_object->addItem(QString::fromStdString(available_objects[i].name));
            if(target_object.ext_joint.parent_obj_pos >=0 && target_object.ext_joint.parent_link_pos >=0)
            {
                if(available_objects[i].name.compare(all_objects[target_object.ext_joint.parent_obj_pos].name) == 0)// Check for parent object
                {
                    ui->parent_object->setCurrentIndex(i); // Set combo box to the parent object
                    on_parent_object_currentIndexChanged(i);
                    for(int j = 0; j < int(available_objects[i].link_vector.size()); j++) // Check for Parent link
                    {
                        if(available_objects[i].link_vector[j].name.compare(all_objects[target_object.ext_joint.parent_obj_pos].link_vector[target_object.ext_joint.parent_link_pos].name) == 0)
                        {
        ...
        if(target_object.ext_joint.parent_obj_pos == -1 && target_object.ext_joint.parent_link_pos == -1)
        {

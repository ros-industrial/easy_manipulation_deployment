...
void NewScene::on_add_object_clicked()
{
    ...
    if(object_window.success)
    {
        ...
        for(int i = 0; i < int(object_window.object.link_vector.size()); i++)
        {
        ...
        for(int i = 0; i < int(object_window.object.joint_vector.size()); i++)
        {
...
void NewScene::add_desc_links(QString OutputFolder, int ee_or_robot) // 0 for robot, 1 for ee
{
    ...
    for (auto it = OutputFolder.toStdString().crbegin() ; it != OutputFolder.toStdString().crend(); ++it)
    {
        if (*it != '/')
        {
    ...
    if(object_name.find("_description") > object_name.length())
    {
        if(ee_or_robot == 0)
        {
            ui->robot_desc_error->setText("<font color='red'>Folder error: Please provide the filepath to the robot description folder</font>");
        }
        else
        {
            ui->ee_desc_error->setText("<font color='red'>Folder error: Please provide the filepath to the End Effector description folder</font>");
        }
        return;
    }
    else
...
//         robot_links = GetLinks(urdf_filename);
           for(int i = 0; i < int(gui_environment.object_names.size()); i++)
           {
...
void NewScene::on_load_robot_desc_clicked()
{
    ...
    OutputFolder = QFileDialog::getExistingDirectory(0, ("Select Robot Description Folder"), QDir::currentPath());
...
{
    QString OutputFolder = QFileDialog::getExistingDirectory(0, ("Select End Effector Description Folder"), QDir::currentPath());
    if(!OutputFolder.isEmpty())
    {
...
{
    int total_objects = gui_environment.object_names.size() + gui_environment.robot_names.size()  + gui_environment.ee_names.size() ;
    if(total_objects> 1)
    {
        ...
        combine_names.insert( combine_names.end(), gui_environment.robot_names.begin(), gui_environment.robot_names.end() );
        combine_names.insert( combine_names.end(), gui_environment.ee_names.begin(), gui_environment.ee_names.end() );
        combine_names.insert( combine_names.end(), gui_environment.object_names.begin(), gui_environment.object_names.end() );

        std::vector<std::vector<std::string>> combine_links;
        combine_links.insert(combine_links.end(), gui_environment.robot_links.begin(), gui_environment.robot_links.end());
        combine_links.insert(combine_links.end(), gui_environment.ee_links.begin(), gui_environment.ee_links.end());
        combine_links.insert(combine_links.end(), gui_environment.object_links.begin(), gui_environment.object_links.end());
        ...
        {
            ui->parent_link_list->addItem(QString::fromStdString(external_joint_window.joint.parent_link));
    ...
    {
        ui->ext_joint_error->setText(" <font color='red'>Error: You need at least 2 Objects to create an external joint</font>");
    }
}
...
void NewScene::on_delete_object_clicked()
{
    ...
    if(list.size() !=0)
    {
        gui_environment.environment.object_vector.erase(gui_environment.environment.object_vector.begin() + list[0].row());
...
void NewScene::on_ext_joint_list_itemDoubleClicked(QListWidgetItem *item)
{
    ...
    external_joint_window.LoadExternalJoint(gui_environment.environment.ext_joint_vector[pos]);


    external_joint_window.exec();

    ui->parent_link_list->item(pos)->setText(QString::fromStdString(external_joint_window.joint.parent_link));
    ui->parent_obj_list->item(pos)->setText(QString::fromStdString(external_joint_window.parent_object_str));
    ui->child_obj_list->item(pos)->setText(QString::fromStdString(external_joint_window.child_object_str));
    ui->child_link_list->item(pos)->setText(QString::fromStdString(external_joint_window.joint.child_link));
...
void NewScene::on_del_ext_joint_clicked()
{
    ...
    if(list.size() !=0)
    {
        gui_environment.environment.ext_joint_vector.erase(gui_environment.environment.ext_joint_vector.begin() + list[0].row());
...
int NewScene::ErrorCheckOrigin(int robot_or_ee)
{
    ...
    if(robot_or_ee == 0)
    {
        if(ui->robot_x->text().isEmpty() || ui->robot_y->text().isEmpty() || ui->robot_z->text().isEmpty()||ui->robot_roll->text().isEmpty() || ui->robot_pitch->text().isEmpty() || ui->robot_yaw->text().isEmpty())
        {
            ui->robot_desc_error->setText(" <font color='red'>Error: XYZ or RPY values for Robot not completely filled.</font>");
            num_errors++;
        }
        else
        {
            ...
            if(float_validator->validate(input_x,i) != QValidator::Acceptable ||float_validator->validate(input_y,i) != QValidator::Acceptable ||float_validator->validate(input_z,i) != QValidator::Acceptable ||float_validator->validate(input_roll,i) != QValidator::Acceptable ||float_validator->validate(input_pitch,i) != QValidator::Acceptable ||float_validator->validate(input_yaw,i) != QValidator::Acceptable )
            {
                ui->robot_desc_error->setText(" <font color='red'>Type Error: Robot XYZ and RPY need to be floats</font>");
                num_errors++;
            }
            else
            {
    ...
    {
        if(ui->ee_x->text().isEmpty() || ui->ee_y->text().isEmpty() || ui->ee_z->text().isEmpty()||ui->ee_roll->text().isEmpty() || ui->ee_pitch->text().isEmpty() || ui->ee_yaw->text().isEmpty())
        {
            ui->ee_desc_error->setText(" <font color='red'>Error: XYZ or RPY values for End Effector not completely filled.</font>");
            num_errors++;
        }
        else
        {
            ...
            if(float_validator->validate(input_x,i) != QValidator::Acceptable ||float_validator->validate(input_y,i) != QValidator::Acceptable ||float_validator->validate(input_z,i) != QValidator::Acceptable ||float_validator->validate(input_roll,i) != QValidator::Acceptable ||float_validator->validate(input_pitch,i) != QValidator::Acceptable ||float_validator->validate(input_yaw,i) != QValidator::Acceptable )
            {
                ui->ee_desc_error->setText(" <font color='red'>Type Error: End Effector XYZ and RPY need to be floats</font>");
                num_errors++;
            }
            else
            {
...
void NewScene::LoadEnvironment(GUIEnvironment input_environment)
{
    if(input_environment.robot_loaded)
    {
        ...
        {
            ui->robot_desc_label->setText(QString::fromStdString(input_environment.environment.robot_vector[0].filepath));
            ...
            ui->robot_roll->setText(QString::number(input_environment.environment.robot_vector[0].origin.roll));
            ui->robot_pitch->setText(QString::number(input_environment.environment.robot_vector[0].origin.pitch));
            ui->robot_yaw->setText(QString::number(input_environment.environment.robot_vector[0].origin.yaw));
        }
    }
    else
    {
    ...
    if(input_environment.ee_loaded)
    {
        ...
        {
            ui->ee_desc_label->setText(QString::fromStdString(input_environment.environment.ee_vector[0].filepath));
            ...
            ui->ee_roll->setText(QString::number(input_environment.environment.ee_vector[0].origin.roll));
            ui->ee_pitch->setText(QString::number(input_environment.environment.ee_vector[0].origin.pitch));
    ...
    {
        if(input_environment.parent_objects.size() == input_environment.child_objects.size() && input_environment.parent_objects.size() == input_environment.ext_joint_names.size())
        {
            for(int j = 0; j < input_environment.ext_joint_names.size(); j ++)
            {
                ...
                ui->child_link_list->addItem(QString::fromStdString(input_environment.environment.ext_joint_vector[j].child_link));
                ui->parent_obj_list->addItem(QString::fromStdString(input_environment.parent_objects[j]));
                ui->parent_link_list->addItem(QString::fromStdString(input_environment.environment.ext_joint_vector[j].parent_link));
            }
        }
    }

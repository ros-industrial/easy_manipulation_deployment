...
{
    ui->setupUi(this);
    for(int i = 0; i < int(supported_types.size()) ; i++)
    {
...
int AddEndEffector::LoadAvailableEE(Robot robot)
{
    ...
    else
    {
        for(auto & filepath : boost::filesystem::directory_iterator( boost::filesystem::current_path()))
        {
            ...
            for (auto it =  filepath.path().string().crbegin() ; it !=  filepath.path().string().crend(); ++it)
            {
                ...
                else
                {
                    ...
                    for(auto & filepath_model : boost::filesystem::directory_iterator( boost::filesystem::current_path()))
                    {
                        bool is_model = false;
                        ...
                        for (auto it =  filepath_model.path().string().crbegin() ; it !=  filepath_model.path().string().crend(); ++it)
                        {
                            if (*it != '/')
                            {
        ...
        {
            for(int j = 0; j < int(available_ee[i].size()); j++)
            {
                total_ee++;
            }
        }
...
void AddEndEffector::LoadExistingEE(EndEffector ee_input)
{
    ...
    for(int brand =0; brand < int(available_brands.size()); brand ++)
    {
        if(ee.brand.compare(available_brands[brand]) == 0)
        {
            ...
            for(int model = 0; model < int(available_ee[brand].size()); model++)
            {
                if(ee.name.compare(available_ee[brand][model].name) == 0)
                {
                    ui->ee_model->setCurrentIndex(model);
                    for(int link = 0; link < int(available_ee[brand][model].ee_links.size()); link++)
                    {
...
void AddEndEffector::on_ee_brand_currentIndexChanged(int index)
{
    ...
    ui->ee_model->clear();
    for(int i = 0; i < int(available_ee[index].size()); i++)
    {
...
void AddEndEffector::on_ee_model_currentIndexChanged(int index)
{
    ...
    {
        for(int i = 0; i < int(available_ee[ui->ee_brand->currentIndex()][ui->ee_model->currentIndex()].ee_links.size()); i++)
        {
            ui->ee_links->addItem(QString::fromStdString(available_ee[ui->ee_brand->currentIndex()][index].ee_links[i]));
        }
    }
...
void AddEndEffector::on_ee_type_currentIndexChanged(int index)
{
    ...
    if(supported_attributes[index].size() == 1)
    {
        ...
        for(int i = 0; i < int(supported_attributes[index][0].size()); i++)
        {
    ...
    else if(supported_attributes[index].size() == 2)
    {
        ...
        for(int i = 0; i < int(supported_attributes[index][1].size()); i++)
        {
...
int AddEndEffector::ErrorCheckOrigin()
{
    ...
    if(ui->include_origin->isChecked())
    {
        ...
        {
            ui->errorlist->append(" <font color='red'> All Origin Fields are empty.Uncheck the Origin selection to disable the option. \n </font>");
            num_errors++;
        }
        else
        {
            ...
            {
                ui->errorlist->append("<font color='red'> XYZ values not complete. Leave it all blank for default values. \n </font>");
                num_errors++;
            }
            else
            {
                if(all_full_xyz)
                {
                    ...
                    {
                        ui->errorlist->append("<font color='red'> Type Error: XYZ need to be floats \n </font>");
                        num_errors++;
                    }
                    else
                    {
            ...
            {
                ui->errorlist->append("<font color='red'> RPY values not complete. Leave it all blank for default values </font>");
                num_errors++;
            }
            else
            {

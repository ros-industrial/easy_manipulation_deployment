...
#include "replacewarning.h"
#include "addscene.h"

#include "generate_yaml.h"
#include "object_xacro_parser.h"
#include "object_package_parser.h"
#include "scene_check.h"
#include "scene_parser.h"
#include "scene_xacro_parser.h"
#include "armhand_xacro_parser.h"
...
void SceneSelect::generate_scene_package(boost::filesystem::path scene_filepath, std::string scene_name, int ros_ver)
{
...
void SceneSelect::generate_scene_files(Scene scene)
{
    ...
    boost::filesystem::path launch_path(workcell_path.string() + "/easy_manipulation_deployment/workcell_builder/examples/ros" + std::to_string(workcell.ros_ver) + "/launch");
    ...
    else if(workcell.ros_ver == 2) //ROS2
    {
        ...
        find_replace("demo_interim.launch.py", "demo_interim2.launch.py", "base_link_name", scene.robot_vector[0].base_link);
        find_replace("demo_interim2.launch.py", "demo.launch.py", "moveit_config_name", scene.robot_vector[0].name + "_moveit_config");
    }
}
void SceneSelect::refresh_scenes(int latest_scene)
{
    ...
    if(workcell.scene_vector.size() > 0)// There are scenes in the workcell
    {
        ...
        for(int scene = 0; scene < int(workcell.scene_vector.size()); scene++)
        {
...
void SceneSelect::on_edit_scene_clicked()
{
    ...
    if(ui->scene_list->currentIndex() >=0) // Make sure that there are scenes to select
    {
        ...
        {
            if(!load_scene_from_yaml(&curr_scene)) //if scene.loaded is not true, generate scene from yaml
            {
        ...
        if(scene_window.success)
        {
            ...
            {
                boost::filesystem::current_path(scenes_path);
                boost::filesystem::path scene_yaml_path(scenes_path.string() + "/" + scene_window.scene.name);
                if(boost::filesystem::exists(scene_window.scene.name))// Scene name nvr change
                {
                    //Replace the current environment yaml
                    GenerateYAML::generate_yaml(scene_window.scene, scene_yaml_path.string(), scenes_path, assets_path);
                ...
                ui->error_workcell->append("<font color='orange'> Warning: Scene has been edited. Previous yaml file removed. Click \" Generate YAML \" again to Generate YAML. </font>");
                refresh_scenes(ui->scene_list->currentIndex());
            }
        }
        else
...
void SceneSelect::on_generate_yaml_clicked()
{
    ...
    if(ui->scene_list->currentIndex() >=0) // Make sure that there are scenes to select
    {
        boost::filesystem::path scene_yaml_path(scenes_path.string() + "/" +workcell.scene_vector[ui->scene_list->currentIndex()].name);
        Scene target_scene = workcell.scene_vector[ui->scene_list->currentIndex()];

        if(!target_scene.loaded) // No scene currently loaded
        {
            if(check_yaml()) // If yaml file is in folder, it might get replaced by new scene configuration
            {
                ui->error_workcell->append("<font color='orange'> ERROR: No changes were made to existing scene, so environment yaml remains the same </font>");
                return;
            }
            else // No yaml in scene folder, no loaded scene from created
            {
                ui->error_workcell->append("<font color='red'> ERROR: No Existing YAML file found, and no new scene generated. </font>");
                return;
            }
        }
        else
        {
            if(check_yaml()) // If yaml file is in folder, it might get replaced by new config
            {
                ReplaceWarning replace_window;
                replace_window.setWindowTitle("Edit Scene");
                replace_window.set_label("Warning: Environment yaml currently exists. Current environment yaml will be replaced. Continue?");
                replace_window.setModal(true);
                replace_window.exec();
                if(replace_window.decision) // user allows for replacing of current yaml file
                {
                    GenerateYAML::generate_yaml(target_scene, scene_yaml_path.string(), scenes_path, assets_path);
                    ui->error_workcell->clear();
...
bool SceneSelect::check_scene()
{
    ...
    if(has_yaml)
    {
        ui->error_workcell->append("<font color='green'>[Scene Status] Environment YAML present </font>");
    }
    else
    {
        ui->error_workcell->append("<font color='orange'>[Scene Status] Environment YAML not present </font>");
    }

    if(files_loaded_proper)
    {
        ui->error_workcell->append("<font color='green'>[Scene Status] All files generated properly </font>");
    }

    if(has_yaml && files_loaded_proper)
    {
        ui->error_workcell->append("<font color='green'>[Scene Status] Scene generation complete. You may exit this application </font>");
    }
    if(!has_yaml && files_loaded_proper)
    {
        ui->error_workcell->append("<font color='orange'>[Scene Status] Scene generation complete, but without environment yaml you cannot edit this scene after exit. </font>");
    }
...
bool SceneSelect::check_files()
{
    boost::filesystem::current_path(scenes_path);// in scenes folder
    boost::filesystem::current_path((ui->scene_list->currentText()).toStdString());
    if(!boost::filesystem::exists("launch")||!boost::filesystem::exists("urdf")||!boost::filesystem::exists("CMakeLists.txt")||!boost::filesystem::exists("package.xml"))
    {
        ui->error_workcell->append("<font color='red'>[Scene Status] ERROR: Files not generated properly </font>");

        if(!boost::filesystem::exists("launch"))
        {
            ui->error_workcell->append("<font color='red'>[Scene Status] ERROR: launch folder missing </font>");
        }
        if(!boost::filesystem::exists("urdf"))
        {
            ui->error_workcell->append("<font color='red'>[Scene Status] ERROR: urdf folder missing </font>");
        }
        if(!boost::filesystem::exists("CMakeLists.txt"))
        {
            ui->error_workcell->append("<font color='red'>[Scene Status] ERROR: CMakeLists.txt missing </font>");
        }
        if(!boost::filesystem::exists("package.xml"))
        {
            ui->error_workcell->append("<font color='red'>[Scene Status] ERROR: Package.xml missing </font>");
        }
        boost::filesystem::current_path(scenes_path);
        return false;
    }
    else
    {
        ...
        if(!boost::filesystem::exists("demo.rviz") || !boost::filesystem::exists("demo.launch.py"))
        {
            ...
            {
                ui->error_workcell->append("<font color='red'>[Scene Status] ERROR: demo.rviz missing </font>");
            }
            if(!boost::filesystem::exists("demo.launch.py"))
            {
                ui->error_workcell->append("<font color='red'>[Scene Status] ERROR: demo.launch.py missing </font>");
            }
            boost::filesystem::current_path(scenes_path);
            return false;
        }
        ...
        if(!boost::filesystem::exists("arm_hand.srdf.xacro") || !boost::filesystem::exists("scene.urdf.xacro"))
        {
            ui->error_workcell->append("<font color='red'>[Scene Status] ERROR: Files not generated properly </font>");
            if(!boost::filesystem::exists("arm_hand.srdf.xacro"))
            {
                ui->error_workcell->append("<font color='red'>[Scene Status] ERROR: arm_hand.srdf.xacro missing </font>");
            }
            if(!boost::filesystem::exists("scene.urdf.xacro"))
            {
                ui->error_workcell->append("<font color='red'>[Scene Status] ERROR: scene.urdf.xacro missing </font>");
            }
            boost::filesystem::current_path(scenes_path);
            return false;
        }
...
void SceneSelect::on_generate_files_clicked()
{
    ...
    if(ui->scene_list->currentIndex() >=0) // Make sure that there are scenes to select
    {
        ...
        {   
            ...
            boost::filesystem::current_path(workcell_path.string() + "/assets/environment/" + object.name + "_description/urdf");
...
bool SceneSelect::load_scene_from_yaml(Scene * input_scene)
{
    ...
    {
        ui->error_workcell->append("<font color='red'> Something went wrong with the yaml file. Please Generate it again </font>");
        return false;
    }

    //TODO: add error catch if trying to create directory with same name, or maybe add a number to it, eg table1, table2.
    YAML::Node objects;
    ...
    if(has_objects) // We need to do this because the object field needs to load before the ext joint field, and it currently has a random load order
    {
        for(YAML::iterator objects_it = objects.begin(); objects_it != objects.end(); ++objects_it)
        {
            ...
            for(YAML::iterator in_object_it = objects_it->second.begin(); in_object_it != objects_it->second.end(); ++in_object_it)
            {
                ...
                if(in_object_it->first.as<std::string>().compare("joints") == 0)
                {
                    ...
                    SceneParser::LoadJointsFromYAML(&temp_joint_vector,temp_object.link_vector, in_object_it->second);
            ...
            for(YAML::iterator in_ext_joint_it  = ext_joint.begin(); in_ext_joint_it != ext_joint.end(); ++in_ext_joint_it)
            {
                ...
                if(in_ext_joint_it->first.as<std::string>().compare("child_link") == 0)
                {
                    ...
                    for(int i = 0; i < int(temp_object.link_vector.size()); i++)
                    {
        ...
        int counter = 0;
        for(YAML::iterator ext_joints_it = ext_joints.begin(); ext_joints_it != ext_joints.end(); ++ext_joints_it)
        {
            ...
            for(YAML::iterator in_ext_joints_it = in_ext_joints.begin(); in_ext_joints_it != in_ext_joints.end(); ++in_ext_joints_it)
            {
                if(in_ext_joints_it->first.as<std::string>().compare("parent object") == 0)
                {
                    ...
                    {
                        for(int i = 0; i < int(input_scene->object_vector.size()); i++)
                        {
                ...
                if(in_ext_joints_it->first.as<std::string>().compare("parent link") == 0)
                {
                    ...
                    if(parent_obj_pos >= 0)
                    {
                        for(int i = 0; i < int(input_scene->object_vector[parent_obj_pos].link_vector.size()); i++)
                        {
                            if(parent_link.compare(input_scene->object_vector[parent_obj_pos].link_vector[i].name) == 0)
                            {
                ...
                if(in_ext_joints_it->first.as<std::string>().compare("origin") == 0)
                {
                    ...
                    SceneParser::LoadOriginFromYAML(&(input_scene->object_vector[counter].ext_joint.origin),in_ext_joints_it->second);
                }
                if(in_ext_joints_it->first.as<std::string>().compare("axis") == 0)
                {
                    input_scene->object_vector[counter].ext_joint.axis.is_axis = true;
                    SceneParser::LoadAxisFromYAML(&(input_scene->object_vector[counter].ext_joint.axis),in_ext_joints_it->second);
                }
            }
            counter++;
        }
...
void SceneSelect::on_back_clicked()
{
    if(workcell.scene_vector[ui->scene_list->currentIndex()].loaded && !check_yaml())
    {
        ReplaceWarning replace_window;
        replace_window.setWindowTitle("Edit Scene");
        replace_window.set_label("Warning: Currently loaded scene is not saved. All progress will be lost. Generate yaml file before going back.");
        replace_window.setModal(true);
        replace_window.exec();

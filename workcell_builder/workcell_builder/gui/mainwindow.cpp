...
{
    QString workcell_file = QFileDialog::getExistingDirectory(this, "Target workcell project destination",QDir::homePath());
...
void MainWindow::on_ros_version_currentIndexChanged(int index)
{
    ...
    ui->ros_distro->clear();
    for(int i = 0; i < int(ros_dist[index].size()); i ++)
    {
...
bool MainWindow::is_good_scene(boost::filesystem::path original_path, std::string scene_name)
{
    ...
    if(!boost::filesystem::exists("urdf")||!boost::filesystem::exists("environment.yaml")||!boost::filesystem::exists("CMakeLists.txt")||!boost::filesystem::exists("package.xml"))
    {
        boost::filesystem::current_path(original_path);
        return false;
    }

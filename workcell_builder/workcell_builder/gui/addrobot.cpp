...
void AddRobot::LoadExistingRobot(Robot robot_input)
{
    ...
    //Populate Brand
    for(int brand =0; brand < int(available_brands.size()); brand ++)
    {
        if(robot.brand.compare(available_brands[brand]) == 0)
        {
            ui->robot_brand->setCurrentIndex(brand);
            for(int model = 0; model < int(available_robots[brand].size()); model++)
            {
                if(robot.name.compare(available_robots[brand][model].name) == 0)
                {
                    ui->robot_model->setCurrentIndex(model);
                    for(int link = 0; link < int(available_robots[brand][model].robot_links.size()); link++)
                    {
...
int AddRobot::LoadAvailableRobots()
{
    ...
    else
    {
        ...
        for(auto & filepath : boost::filesystem::directory_iterator( boost::filesystem::current_path()))
        {
            ...
            for (auto it =  filepath.path().string().crbegin() ; it !=  filepath.path().string().crend(); ++it)
            {
                if (*it != '/')
                {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if(moveit_configs.size() < descriptions.size())
                        {
                            ...
                            {
                                if (std::find(moveit_configs.begin(), moveit_configs.end(), descriptions[i]) != moveit_configs.end())
                                {
                                    brand_robot_vector.push_back(LoadRobot(descriptions[i]+ "_description", temp_brand));
                                }
                            }
                        }
                        else{
                            ...
                            {
                                if (std::find(descriptions.begin(), descriptions.end(), moveit_configs[i]) != descriptions.end())
                                {
                                    brand_robot_vector.push_back(LoadRobot(moveit_configs[i] + "_description", temp_brand));
                                }
                            }
                        }
                    }
        ...
        for(int i3 = 0; i3 < int(available_brands.size()); i3++)
        {
        ...
        int total_count = 0;
        for(int i = 0; i < int(available_brands.size()); i ++)
        {
            for(int i2 = 0; i2 < int(available_robots[i].size()); i2++)
            {
                total_count++;
            }
        }
...
std::vector<Robot> AddRobot::LoadUR(std::vector<std::string> robot_list) // load ur robots based on the way it is structured
{
...
void AddRobot::on_robot_brand_currentIndexChanged(int index)
{
    ...
    for(int i = 0; i < int(available_robots[index].size()); i++)
    {
...
void AddRobot::on_robot_model_currentIndexChanged(int index)
{
    ...
    for(int i = 0; i < int(available_robots[ui->robot_brand->currentIndex()][ui->robot_model->currentIndex()].robot_links.size()); i++)
    {
        ui->robot_links->addItem(QString::fromStdString(available_robots[ui->robot_brand->currentIndex()][index].robot_links[i]));
        ui->robot_ee_link->addItem(QString::fromStdString(available_robots[ui->robot_brand->currentIndex()][index].robot_links[i]));
    }
...
int AddRobot::ErrorCheckOrigin()
{
    ...
    if(ui->include_origin->isChecked())
    {
        ...
        {
            ui->errorlist_2->append(" <font color='red'> All Origin Fields are empty.Uncheck the Origin selection to disable the option. \n </font>");
            num_errors++;
        }
        else
        {
            ...
            {
                ui->errorlist_2->append("<font color='red'> XYZ values not complete. Leave it all blank for default values. \n </font>");
                num_errors++;
            }
            else
            {
                if(all_full_xyz)
                {
                    ...
                    {
                        ui->errorlist_2->append("<font color='red'> Type Error: XYZ need to be floats \n </font>");
                        num_errors++;
                    }
                    else
                    {
            ...
            {
                ui->errorlist_2->append("<font color='red'> RPY values not complete. Leave it all blank for default values </font>");
                num_errors++;
            }
            else
            {
                if(all_full_rpy)
                {
                    ...
                    {
                        ui->errorlist_2->append(" <font color='red'> Type Error: RPY need to be floats </font>");
                        num_errors++;
                    }
                    else
                    {

...
{
    int num_errors = 0;
    bool all_empty_inertial = (ui->ixx->text().isEmpty() && ui->ixy->text().isEmpty() && ui->ixz->text().isEmpty() && ui->iyy->text().isEmpty() && ui->iyz->text().isEmpty() && ui->izz->text().isEmpty());
    bool all_full_inertial = (!ui->ixx->text().isEmpty() && !ui->ixy->text().isEmpty() && !ui->ixz->text().isEmpty() && !ui->iyy->text().isEmpty() && !ui->iyz->text().isEmpty() && !ui->izz->text().isEmpty());

    if(all_empty_inertial)
    {
        ui->errorlist->append("Some inertia fields are not filled up. All inertia fiekds are required to be filled.");
        num_errors++;
    }
    ...
    else
    {
        ...
        else
        {
            if(all_full_inertial)
            {
                ...
                if(float_validator->validate(input_ixx,i) != QValidator::Acceptable ||float_validator->validate(input_ixy,i) != QValidator::Acceptable ||float_validator->validate(input_ixz,i) != QValidator::Acceptable || float_validator->validate(input_iyy,i) != QValidator::Acceptable ||float_validator->validate(input_iyz,i) != QValidator::Acceptable ||float_validator->validate(input_izz,i) != QValidator::Acceptable)
                {
